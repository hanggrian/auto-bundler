...
internal enum class BundleValueType(val typeName: TypeName, private val methodName: String) {
    ...
    CHARSEQUENCE_ARRAYLIST(
        ParameterizedTypeName.get(ArrayList::class.java, CharSequence::class.java),
        "CharSequenceArrayList"
    ),
    PARCELABLE(CLS_PARCELABLE, "Parcelable"),
    PARCELABLE_ARRAY(ArrayTypeName.of(CLS_PARCELABLE), "ParcelableArray"),
    PARCELABLE_ARRAYLIST(
        ParameterizedTypeName.get(ClassName.get(ArrayList::class.java), CLS_PARCELABLE),
        "ParcelableArrayList"
    ),
    PARCELABLE_SPARSEARRAY(
        ParameterizedTypeName.get(CLS_SPARSE_ARRAY, CLS_PARCELABLE),
    ...
    companion object {
        ...
        fun valueOf(fieldElement: Element, typeUtils: Types): BundleValueType {
            ...
            MoreTypes.asTypeElement(fieldElement.asType()).annotationMirrors
                .filter { TypeName.get(it.annotationType) == CLS_PARCEL }
            ...
            throw RuntimeException(
                String.format(
                    ...
                    fieldElement.enclosingElement.simpleName
                )
            )
        }
    }
