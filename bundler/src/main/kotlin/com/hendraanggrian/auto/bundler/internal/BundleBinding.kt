package com.hendraanggrian.auto.bundler.internal
...
abstract class BundleBinding private constructor(
    ...
    protected fun getShort(key: String, defaultValue: Short): Short =
    ...
    ): Array<CharSequence>? = findValue(key, defaultValue) { source.getCharSequenceArray(key) }

    protected fun getCharSequenceArrayList(
        key: String,
        defaultValue: ArrayList<CharSequence>?
    ): ArrayList<CharSequence>? =
        findValue(key, defaultValue) { source.getCharSequenceArrayList(key) }

    protected fun <T : Parcelable> getParcelable(key: String, defaultValue: T?): T? =
        findValue(key, defaultValue) { source.getParcelable(key) }

    protected fun <T : Parcelable> getParcelableArray(
        key: String,
        defaultValue: Array<T>?
    ): Array<T>? = findValue(key, defaultValue) { source.getParcelableArray(key) as Array<T> }

    protected fun <T : Parcelable> getParcelableArrayList(
        key: String,
        defaultValue: ArrayList<T>?
    ): ArrayList<T>? = findValue(key, defaultValue) { source.getParcelableArrayList(key) }

    protected fun <T : Parcelable> getSparseParcelableArray(
        key: String,
        defaultValue: SparseArray<T>?
    ): SparseArray<T>? = findValue(key, defaultValue) { source.getSparseParcelableArray(key) }
    ...
    ): ArrayList<String>? = findValue(key, defaultValue) { source.getStringArrayList(key) }
