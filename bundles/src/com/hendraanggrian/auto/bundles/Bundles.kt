...
import com.hendraanggrian.auto.bundles.internal.BundleBinding
...
object Bundles {
    private const val TAG = "Bundles"
    private var BINDINGS: MutableMap<String, Constructor<BundleBinding>>? = null
    private var DEBUG = false

    /** When set to true, will print Bundles operation in debug level. */
    ...
    private fun createBinding(cls: Class<*>, clsSuffix: String, paramTypes: Array<Class<*>>, params: Array<*>):
        BundleBinding {
        debug("Looking up constructor for ${cls.simpleName}")
        val constructor = findBindingConstructor(cls, clsSuffix, paramTypes)
        if (constructor == null) {
            debug("Ignored because no constructor was found in ${cls.simpleName}")
    ...
    private fun findBindingConstructor(cls: Class<*>, clsSuffix: String, paramTypes: Array<Class<*>>):
        ...
        val bindingKey = cls.name + clsSuffix + paramTypes[0].simpleName
        ...
        try {
            ...
            debug("Not found. Trying superclass ${supercls.name}")
        ...
        BINDINGS!![bindingKey] = checkNotNull(binding) {
            "Unable to find preferences binding, is `bundles-compiler` correctly installed?"
        }
        return binding
    }
}
